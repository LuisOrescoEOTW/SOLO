...
import { deleted, get, getRelaciones } from "../../redux/slices/thunks";
...
export const General = ({ tabla, columnas, relaciones, auxTablas }: Props) => {
  //Leer
  ...
  useEffect(() => {
    if (isAuthenticated) {
      auxTablas.forEach((tablaAux) => {
        dispatch(get(tablaAux, actionCreatorMap));
      });
    }
  }, [dispatch, isAuthenticated, auxTablas]);
  useEffect(() => {
    if (isAuthenticated) {
      if (relaciones) {
